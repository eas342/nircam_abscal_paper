...
\begin{itemize}
\item {\bf Age:} For the methods to work, the solar-mass stars must be on the main sequence. This means the age of the cluster must be between 0.2 Gyr (??) and 8 Gyr (??) using Dartmouth evolutionary models \citep{dotter2008dartmouth}.
\item {\bf Extinction} Extinction should be as small as possible to minimize the effects of extinction correction to photometry. This generally means E(B-V) $\lesssim < 0.1$.
\item {\bf Distance Modulus} The cluster must be distant enough to not saturate NIRCam's wide filters in full frame mode.
\item {\bf Metallicity}
\item {\bf Compactness}
