...
We discuss a program to identify solar analog calibrators at the low end of the flux scale that will not saturate wide-band NIRCam imaging in full frame mode.
We find that solar analog stars in NGC~2420 and NGC~2506 have the best properties for calibration in terms of extinction, age, distance and metallicity.
We include a catalog of sources and absolutely calibrated photometry in the $g$, $r$, $i$, $z$, $J$, $H$, $K$ and IRAC1 3.6 $\mu$m bands.
We also provide a catalog of solar analog candidates that are selected from optical spectroscopy that will serve as faint calibrator sources for JWST imaging.
These faint calibrators serve wide band full frame imaging of current and future ground-based imagers.
\end{abstract}
...
\item {\bf Distance Modulus} The cluster must be distant enough to not saturate NIRCam's wide filters in full frame mode. The most easily saturated wide band filter for G stars is the F090W, which corresponds to a G2~V star of 15.9 in the $K$ band.
While the F150W2 ``double-wide'' filter is more easily saturated than F090W, this is filter is unlikely to be used on faint Galaxies because (someone from deep field prog remind me?).
This means the true distance modulus $(m - M)_0$ should be 12.6 for a solar absolute K=3.28 \citep{binney1998book}.
...
NGC 2506 is the next-best fits and have somewhat complimentary visibility windows, shown in Figure \ref{fig:clusterVis}. NGC 2420 and NGC 6811 have -0.05 $\le$ [Fe/H] $\le$ 0.04.
